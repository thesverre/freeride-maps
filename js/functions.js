...
function addVarsom(latLng) {
	...
	$.get('php/fetch.php?type=varsom&lat=' + lat + "&lon=" + lon + "&startdate=" + date + "&enddate=" + date, function(result) {
		...
		if (result.AvalancheDanger) {
			...
			var html = '<a target="_blank" href="http://varsom.no/Snoskred/' + result.RegionName.replace('ø', 'o').replace('å', 'a').replace('æ', 'a') + '">'  + '<span class="danger level' + result.DangerLevel +'" title="'+result.DangerLevelName + '">' + result.DangerLevel + '</span></a>';
			tag.html(html);
		}
	});
}
...
function addLargeContainer(clickedLocation, path, name) {
    ...
    html += '</div>'
    addTwitter( name);
		addInstagramPictures(name);
    return html;
}

function addInstagramPictures(name) {
	$.get('php/fetch.php?type=instagram&name=' + name, function(data) {
		$('#instagram').html(data);
	})
}
...
function activateLayer(layerId) {
	...
	if (layerId == 'clouds_precipitation_regional') {
	    ...
	    loadWMS(map, 'http://gis2.ngi.no/arcgistemp/rest/services/Skred/BratteOmrApp/MapServer/export?dpi=96&transparent=true&format=png24&layers=show%3A0&bboxSR=3857&imageSR=3857&size=1024%2C1024&f=image', {
	        tileWidth : 1024,
	        tileHeight : 1024,
	        opacity : 0.5
	    });
